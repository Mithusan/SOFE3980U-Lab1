...
public class Binary
{
	...
	*/
	...
	public static Binary bitWiseMultiply(Binary num1, Binary num2)
	{
		...
		//If the first number is larger then value will be of the first number
		//Else it will be the second number
		Binary largeNum = (ind1>=ind2)? num1:num2;
		//If the first number is equal to the calculated 'largeNum'
		//It will be the second number if not the first number
		Binary smallNum = (largeNum==num1)? num2:num1;
		//Assigns objects to variables
		num1 = largeNum;
		num2 = smallNum;

		//loops until no digits remain
		for(int i=smallNum.number.length()-1; i>=0; i--)
		{
			//If current iteration value is equal to 1
			if(smallNum.number.charAt(i)=='1'){
				//Sends to add method to add larger number
				//to result
				result = add(result,num1);
			}
			//If not will add 0 to calculation for current iteration
			num1.number+="0";
		}
